...
from utils import PrototypePostprocessor, subsample_dataset, MergedDataset
# TODO: experiment with mahalanobis distance instead of euclidean
...
sns.heatmap(prot_usage[usage_order_idx[-1]].reshape(8, 8))
...
def main(
    ...
    if id_dataset == DATASET_CIFAR10:
        ...
        def get_ood_dataset(split="train", augmentation=False):
            if augmentation:
                transforms = [
                    ...
                    tt.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.7, 1.3), shear=10),
                    tt.RandomApply([tt.Resize((32, 32))], p=1.0),
                    tt.CenterCrop(32),
                    tt.ToTensor(),
                    tt.Normalize(mean, std)
        ...
        def get_ood_dataset(split="train", augmentation=False):
            if augmentation:
                transforms = [
                    ToRGB(),
                    #tt.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),
                    tt.RandomAffine(degrees=0, translate=(0.05, 0.05), scale=(0.25, 1.3), shear=10),
                    tt.RandomApply([tt.Resize((32, 32))], p=0.2),
                    tt.CenterCrop(32),
                    tt.ToTensor(),
                    tt.Normalize(mean, std)
                ]
        ...
        def get_id_dataset(split="train"):
            id_transform = tt.Compose([ToRGB(), tt.Resize((224, 224)), tt.ToTensor(), tt.Normalize(mean, std)])
            id_dataset = ImageDatasetFromFile(
                    transform=id_transform,
        ...
        def get_ood_dataset(split=None, augmentation=False):
            """ ImageNet-200 does not have an OOD validation set.
            """
            transforms = [
                ToRGB(),
                #tt.CenterCrop(224),
    ...
    if train_prototypes:
        trainer = pl.Trainer(max_epochs=3, logger=logger, accumulate_grad_batches=50, precision=32, log_every_n_steps=5)
        backbone.eval()
